...
describe('BankingService', () => {
  ...
  it('does not reconcile a transaction belonging to another organization', async () => {
    ...
    await expect(service.reconcile('org-a', 'transaction-id', 'RECONCILED')).rejects.toBeInstanceOf(
      NotFoundException,
    );
    expect(findFirst).toHaveBeenCalledWith({
      where: { id: 'transaction-id', organizationId: 'org-a' },
    });
  });
  ...
  it('sets both opening and current balances when an account is created', async () => {
    ...
    const data = (
      create.mock.calls[0]?.[0] as {
        data: { organizationId: string; currentBalance: Prisma.Decimal; currency: string };
      }
    ).data;
  ...
  it('validates every CSV row before starting an import transaction', async () => {
    ...
    const service = new BankingService({
      bankAccount: { findFirst: jest.fn().mockResolvedValue({ id: 'account', isActive: true }) },
      $transaction: transaction,
    } as never);
    await expect(
      service.importTransactions('org-a', {
