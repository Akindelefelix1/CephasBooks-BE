...
import { IsBoolean, IsDateString, IsEnum, IsNumber, IsOptional, IsString, IsUUID, Length, MaxLength, Min } from 'class-validator';
...
export class CreateBankAccountDto {
  ...
  @ApiPropertyOptional({ default: 'NGN' }) @IsOptional() @IsString() @Length(3, 3) currency?: string;
  @ApiPropertyOptional({ default: 0 }) @Type(() => Number) @IsNumber() openingBalance = 0;
}
export class UpdateBankAccountDto {
  @ApiPropertyOptional() @IsOptional() @IsString() @MaxLength(120) name?: string;
  @ApiPropertyOptional() @IsOptional() @IsString() @MaxLength(120) bankName?: string;
  @ApiPropertyOptional({ enum: BankAccountType }) @IsOptional() @IsEnum(BankAccountType) accountType?: BankAccountType;
  @ApiPropertyOptional() @IsOptional() @IsString() @Length(4, 4) accountNumberLast4?: string;
...
export class ReconcileTransactionDto {
  @ApiProperty({ enum: ReconciliationStatus }) @IsEnum(ReconciliationStatus) status!: ReconciliationStatus;
}
export class ImportTransactionsDto {
  @ApiProperty() @IsUUID() bankAccountId!: string;
  @ApiProperty({ description: 'CSV with date,description,reference,type,amount columns' }) @IsString() csv!: string;
}
...
export class UpdateBankTransactionDto {
  ...
  @ApiPropertyOptional() @IsOptional() @IsString() @MaxLength(100) reference?: string;
  @ApiPropertyOptional({ enum: BankTransactionType }) @IsOptional() @IsEnum(BankTransactionType) type?: BankTransactionType;
